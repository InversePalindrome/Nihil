...
void StartState::update(float deltaTime)
{
	...
	this->background.setTextureRect(sf::IntRect(static_cast<std::size_t>(this->view.getCenter().x - this->view.getSize().x / 2.f), 0u,
		static_cast<std::size_t>(this->view.getSize().x), static_cast<std::size_t>(this->view.getSize().y)));
